...
class SocketInfo:
    def __init__(self, notebook):
        ...
        self.refresh_button = ttk.Button(self.frame, text="Refresh", command=self.get_socket_info)
    ...
    def get_socket_info(self):
        ...
        try:
            ...
            self.info_text.insert(tk.END, f"Host Name: {host_name}\n")
            self.info_text.insert(tk.END, f"IP Address: {ip_address}\n")
            
            self.info_text.insert(tk.END, "\nOpen Sockets:\n")
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                s.bind(("", 0))
                _, port = s.getsockname()
                self.info_text.insert(tk.END, f"Listening on port: {port}\n")
        except Exception as e:
